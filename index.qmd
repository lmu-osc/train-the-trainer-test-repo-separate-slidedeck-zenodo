...
format: 
  ...
  pptx-testing: 
    ...
    output-file: lmu-branded-test.pptx
    date: 03-03-2020 # this works, but had to set up the template correctly to have the date in footer on every slide
    date-format: long # Quarto's auto-formatting of date seems to play nice with pptx
    # footer: hello there footer # footer does nothing
    testing: "Trying a custom field"
    incremental: true
  pptx-working:
    slide-level: 2
    reference-doc: working-template.pptx
    output-file: working-output.pptx
    date: 03-03-2020 # this works, but had to set up the template correctly to have the date in footer on every slide
    date-format: long # Quarto's auto-formatting of date seems to play nice with pptx
    # footer: hello there footer # footer does nothing
    testing: "Trying a custom field"
...
## Testing Slide {background-color="blue"}

::: {.callout-tip}

This is a callout box test

:::

Test citation [@jacksonDailyGrind1990](#references)

# Section Header?

content in section header?
